...
%   model.obsdimPER  = 2*L*D1 + 1*D1 + 1*D2; %for modcurve8newP2
  ...
  LinVar = [0.183 0.639 1.33]*1e10;
  %Variance for the body angular acceleration
  AngVar = [.168 .181 2.20]*1e9;
  ...
  Qbody = [0.001.*ones(1,3) 0.00001.*ones(1,4)];
  if model.statedim <= 7
      Q1 = Qbody;
  elseif model.statedim > 15
      Q1 = [repmat(WingVar,1,8) LinVar AngVar];
  else
      Q1 = [Qbody repmat(WingVar,1,8)];
  end
...
mdlLen = PAR.statedim;
Tnew = zeros(size(state,2),2);

if strcmp(model.Fmode,'predict')
    ...
    for j = 1:size(state,2)
        ...
        [a(1:mdlLen,:),Tnew(j,:)] = recalcIC_Joint(state(1:mdlLen,j),[],PAR,'calc');        
        state(:,j) = a;
    end
...
for j = 1:size(state,2)
    ...
    for k = 1:PAR.numfly
         ...
         [x,y,z] = flymodQ(pp,PAR.params,PAR);
...
for i = 1:size(state,2)
    for k = 1:PAR.numfly
        ...
        %Apply Constraints
        xh_ = xh;
        xh_(4:7) = xh_(4:7)./norm(xh_(4:7));
        
        %keyboard
        if length(xh_) > 7
            xh_(8:11) = xh_(8:11)./norm(xh_(8:11));
            xh_(12:15) = xh_(12:15)./norm(xh_(12:15));

            xh_ = fixRollTPlaneQ(xh_,PAR);
        end
        
        new_state((k-1)*mdlLen+1:k*mdlLen,i) = xh_;
    end
end
%keyboard
% Make sure that all the body quaternions have the same sign
% TEMPORARY FIX because of the double covering of SO(3) by quaternions that
% gets mixed up during conversions in 'fixrollTPlaneQ.m'
qb = state(4:7,1);
[dum,qidx] = max(abs(qb)); 
sgn = sign(qb(qidx));
idx = find(sign(new_state(3+qidx,:)) ~= sgn);
new_state(4:7,idx) = -new_state(4:7,idx);
%==========================================================================
