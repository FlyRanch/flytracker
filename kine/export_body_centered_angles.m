...
%                                             changed to export body-centered angles; changed default save filename

function export_body_centered_angles

[file path no_err] = uiputfile('export_bc.mat', 'Export angles to...');
...
% TEMP TAKEN OUT, REPLACE THIS CODE AFTER THETA,ALPHA DEBUGGING DONE
frames = frame_range(:);
phi_left =      reshape(data.kine.(left_wing).BCdata.angles(1,phi,frame_range),      prod(size(frame_range)), 1);
theta_left =    reshape(data.kine.(left_wing).BCdata.angles(1,theta,frame_range),    prod(size(frame_range)), 1);
alpha_left =    reshape(data.kine.(left_wing).BCdata.params(1, frame_range),                  prod(size(frame_range)), 1);
phi_right =     reshape(data.kine.(right_wing).BCdata.angles(1,phi,frame_range),     prod(size(frame_range)), 1);
theta_right =   reshape(data.kine.(right_wing).BCdata.angles(1,theta,frame_range),   prod(size(frame_range)), 1);
alpha_right =   reshape(data.kine.(right_wing).BCdata.params(1, frame_range),                 prod(size(frame_range)), 1);
% END TEMP
