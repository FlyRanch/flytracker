...
%                                         in a parallell combination, i.e. obs = [ffun(x)]
%                                                                                [hfun(x)]
...
switch ArgDS.type
    ...
    case 'state'
        ...
        InferenceDS.obsdimPER   = model.obsdimPER;            % observation dimension per model
        ...
        %--- ffun/hfun types ---
        InferenceDS.ffun_type = model.ffun_type;              % state transition function type
        ...
        if isfield(model, 'hfun1'),
            InferenceDS.hfun1 = @hfun1_state;
        end
        if isfield(model, 'hfun2'),
            InferenceDS.hfun2 = @hfun2_state;
        end
        if isfield(model, 'cfun'),
            InferenceDS.cfun = @cfun_state;
        end
    ...
    case 'parameter'
        ...
        case 'both'
            ...
            switch model.ffun_type
            case {'nl','nla'}
                switch model.hfun_type
                case 'nl'
                    InferenceDS.hfun_type = 'nl';
                case {'nla','ltv','lti'}
                    InferenceDS.hfun_type = 'nla';
                otherwise
                    error(' [ geninfds::parameter::both ] Unknown GSSM model hfun_type.');
                end
             case 'ltv'
                switch model.hfun_type
                case 'nl'
             ...
             case 'lti'
                ...
                case 'ltv'
                    InferenceDS.hfun_type = 'ltv';
                case 'lti'
                    InferenceDS.hfun_type = 'lti';
                otherwise
                    error(' [ geninfds::parameter::both ] Unknown GSSM hfun_type.');
                end
             otherwise
                  error(' [ geninfds::parameter::both ] Unknown GSSM ffun_type.');
             end
...
function [new_state,InferenceDS] = ffun_state(InferenceDS, state, V, U1)
    ...
    [new_state,InferenceDS.model] = feval(InferenceDS.model.ffun, InferenceDS.model, state, V, U1);

%-------------------------------------------------------------------------------------
function [observ,InferenceDS] = hfun_state(InferenceDS, state, N, U2)
    ...
    [observ,InferenceDS.model] = feval(InferenceDS.model.hfun, InferenceDS.model, state, N, U2);

    %-------------------------------------------------------------------------------------
function [observ,InferenceDS] = hfun1_state(InferenceDS, state, N, U2)
        ...
        [observ,InferenceDS.model] = feval(InferenceDS.model.hfun1, InferenceDS.model, state, N, U2);

        %-------------------------------------------------------------------------------------
        function [observ,InferenceDS] = hfun2_state(InferenceDS, state, N, U2)
            ...
            [observ,InferenceDS.model] = feval(InferenceDS.model.hfun2, InferenceDS.model, state, N, U2);

%--------------------------------------------------------------------------
function [new_state,InferenceDS] = cfun_state(InferenceDS, state)
    ...
    [new_state,InferenceDS.model] = feval(InferenceDS.model.cfun, InferenceDS.model, state);            
%-------------------------------------------------------------------------------------
...
function llh = likelihood_parameter_both(InferenceDS, obs, state, U2, oNoiseDS)
    ...
    for k=1:nov,
        ...
        llh(k) = feval(InferenceDS.model.likelihood, InferenceDS.model, obs(:,k), ext_state_2, ext_U2(:,k), oNoiseDS);
...
function varargout = linearize_parameter_both(InferenceDS, state, V, N, U1, U2, varargin)
    ...
    if ~isempty(state),
        model = feval(model.setparams, model, state, InferenceDS.paramParamIdxVec);   % set parameters acording to state variable
    end
    ...
    for k=1:length(varargin)
        ...
        case 'C'
            ...
            ext_state_2 = feval(model.ffun, model, ext_state_1, ext_proc_noise, ext_U1);
            extC = feval(model.linearize, model, ext_state_2, [], ext_obs_noise, [], ext_U2, 'C');
            extJFW = feval(model.linearize, model, ext_state_1, ext_proc_noise, [], ext_U1, [], 'JFW', InferenceDS.paramParamIdxVec);
            extJHW = feval(model.linearize, model, ext_state_2, [], ext_obs_noise, [], ext_U2, 'JHW', InferenceDS.paramParamIdxVec);
            Ctemp = extC*extJFW + extJHW;
            C(1:dimHO,:) = Ctemp(hfun_idx,:)
            varargout{k} = C;
        ...
        case 'D'
            ...
            ext_state_2 = feval(model.ffun, model, ext_state_1, ext_proc_noise, ext_U1);
            extA = feval(model.linearize, model, ext_state_1, ext_proc_noise, [], ext_U1, [], 'A');
            extB = feval(model.linearize, model, ext_state_1, ext_proc_noise, [], ext_U1, [], 'B');
            extC = feval(model.linearize, model, ext_state_2, [], ext_obs_noise, [], ext_U2, 'C');
            extD = feval(model.linearize, model, ext_state_2, [], ext_obs_noise, [], ext_U2, 'D');
            tempCA = extC*extA;
            tempCB = extC*extB;
            D(1:dimHO,1:dimX) = tempCA(hfun_idx,:);
            D(1:dimHO,dimX+1:dimX+dimU1) = tempCB(hfun_idx,:);
            D(1:dimHO,dimX+dimU1+1:end) = extD(hfun_idx,:);
            varargout{k} = D;
        ...
        case 'H'
            ...
            ext_state_2 = feval(model.ffun, model, ext_state_1, ext_proc_noise, ext_U1);
            extC = feval(model.linearize, model, ext_state_2, [], ext_obs_noise, [], ext_U2, 'C');
            extG = feval(model.linearize, model, ext_state_1, ext_proc_noise, [], ext_U1, [], 'G');
            extH = feval(model.linearize, model, ext_state_2, [], ext_obs_noise, [], ext_U2, 'H');
            tempCG = extC*extG;
            H(1:dimHO,1:dimV) = tempCG(hfun_idx,:);
            H(1:dimHO,dimV+1:end) = extH(hfun_idx,:);
            varargout{k} = H;
...
function varargout = linearize_parameter_bothp(InferenceDS, state, V, N, U1, U2, varargin)
    ...
    for k=1:length(varargin)
        ...
        case 'C'
            ...
            extJFW = feval(model.linearize, model, ext_state_1, ext_proc_noise, [], ext_U1, [], 'JFW', InferenceDS.paramParamIdxVec);
            extJHW = feval(model.linearize, model, ext_state_2, [], ext_obs_noise, [], ext_U2, 'JHW', InferenceDS.paramParamIdxVec);
            C(1:dimFO,:) = extJFW(ffun_idx,:);
            C(dimFO+1:dimFO+dimHO,:) = extJHW(hfun_idx,:);
            varargout{k} = C;
        ...
        case 'D'
            ...
            extC = feval(model.linearize, model, ext_state_2, [], ext_obs_noise, [], ext_U2, 'C');
            extD = feval(model.linearize, model, ext_state_2, [], ext_obs_noise, [], ext_U2, 'D');
            D(1:dimFO,1:dimX) = extA(ffun_idx,:);
            D(1:dimFO,dimX+1:dimX+dimU1) = extB(ffun_idx,:);
            D(dimFO+1:dimFO+dimHO,dimX+dimU1+1:dimX+dimU1+dimX) = extC(hfun_idx,:);
            D(dimFO+1:dimFO+dimHO,dimX+dimU1+dimX+1:end) = extD(hfun_idx,:);
            varargout{k} = D;
        ...
        case 'H'
            ...
            extG = feval(model.linearize, model, ext_state_1, ext_proc_noise, [], ext_U1, [], 'G');
            extH = feval(model.linearize, model, ext_state_2, [], ext_obs_noise, [], ext_U2, 'H');
            H(1:dimFO,1:dimV) = extG(ffun_idx,:);
            H(dimFO+1:dimFO+dimHO,dimV+1:end) = extH(hfun_idx,:);
            varargout{k} = H;
...
function varargout = linearize_parameter_h(InferenceDS, state, V, N, U1, U2, varargin)
    ...
    for k=1:length(varargin),
        ...
        case 'D'
            ...
            extC = feval(model.linearize, model, ext_state_2, [], ext_obs_noise, [], ext_U2, 'C');
            extD = feval(model.linearize, model, ext_state_2, [], ext_obs_noise, [], ext_U2, 'D');
            D(1:dimHO,1:dimX) = extC(hfun_idx,:);
            D(1:dimHO,dimX+1:dimX+dimU2) = extD(hfun_idx,:);
            varargout{k} = D;
        ...
        case 'H'
            H = zeros(InferenceDS.obsdim, InferenceDS.Ndim);
            extH = feval(model.linearize, model, ext_state_2, [], ext_obs_noise, [], ext_U2, 'H');
            H(1:dimHO,1:dimN) = extH(hfun_idx,:);
            varargout{k} = H;
...
function new_state = ffun_joint(InferenceDS, state, V, U1)
    ...
    for k=1:nov,
      InferenceDS.model = feval(InferenceDS.model.setparams, InferenceDS.model, state(dimX+1:end,k), InferenceDS.paramParamIdxVec);    % set model parameter vector
      new_state(1:dimX,k) = feval(InferenceDS.model.ffun, InferenceDS.model, state(1:dimX,k), V(1:dimV,k), U1(:,k));
...
function tran_prior = prior_joint(InferenceDS, nextstate, state, U1, pNoiseDS)
    ...
    tran_prior = feval(pNoiseDS.likelihood, pNoiseDS, dX, pNoiseDS.N);

    for k=1:nov,
        InferenceDS.model = feval(InferenceDS.model.setparams, InferenceDS.model, paramState(:,k), InferenceDS.paramParamIdxVec); % set model parameter vector
        tran_prior(k) = tran_prior(k) * feval(InferenceDS.model.prior, InferenceDS.model, stateNextState(:,k), stateState(:,k), U1(:,k), pNoiseDS.noiseSources{1});
    end
...
function observ = hfun_joint(InferenceDS, state, N, U2)
    ...
    for k=1:nov,
      InferenceDS.model = feval(InferenceDS.model.setparams, InferenceDS.model, state(dimX+1:end,k), InferenceDS.paramParamIdxVec);    % set model parameter vector
      observ(:,k) = feval(InferenceDS.model.hfun, InferenceDS.model, state(1:dimX,k), N(:,k), U2(:,k));
    end
...
function llh = likelihood_joint(InferenceDS, obs, state, U2, oNoiseDS)
    ...
    for k=1:nov,
      InferenceDS.model = feval(InferenceDS.model.setparams, InferenceDS.model, state(dimX+1:end,k), InferenceDS.paramParamIdxVec); ...
      % set model parameter vector
      llh(k) = feval(InferenceDS.model.likelihood, InferenceDS.model, obs(:,k), state(1:dimX,k), U2(:,k), oNoiseDS);
    end
...
function varargout = linearize_joint(InferenceDS, state, V, N, U1, U2, varargin)
    ...
    % Setup temporary model to use for linearization purposes
    model = feval(model.setparams, model, ext_params, paramIdxVec);       % set parameters acording to state variable
    ...
    for k=1:nop,
        ...
        switch varargin{k}
         ...
         case 'A'
            ...
            A(1:dimX,1:dimX) = feval(model.linearize, model, ext_state, ext_V, N, U1, U2, 'A');
            A(1:dimX,dimX+1:dimX+dimW) = feval(model.linearize, model, ext_state, ext_V, N, U1, U2, 'JFW', paramIdxVec);
            varargout{k} = A;

        case 'B'
            B = zeros(InferenceDS.statedim,InferenceDS.U1dim);
            B(1:dimX,:) = feval(model.linearize, model, ext_state, ext_V, N, U1, U2, 'B');
            varargout{k} = B;

        case 'G'
            G = zeros(InferenceDS.statedim,InferenceDS.Vdim);
            G(1:dimX,1:dimV) = feval(model.linearize, model, ext_state, ext_V, N, U1, U2, 'G');
            G(dimX+1:dimX+dimW,dimV+1:dimV+dimW) = eye(dimW);
            varargout{k} = G;

         case 'C'
            C = zeros(InferenceDS.obsdim,InferenceDS.statedim);
            C(:,1:dimX) =  feval(model.linearize, model, ext_state, ext_V, N, U1, U2, 'C');
            C(:,dimX+1:dimX+dimW) = feval(model.linearize, model, ext_state, ext_V, N, U1, U2, 'JHW', paramIdxVec);
            varargout{k} = C;

         case 'D'
            varargout{k} = feval(model.linearize, model, ext_state, ext_V, N, U1, U2, 'D');

         case 'H'
            varargout{k} = feval(model.linearize, model, ext_state, ext_V, N, U1, U2, 'H');
...
function varargout = linearize_generic(InferenceDS, state, V, N, U1, U2, varargin)
  ...
  for k=1:nop,
      ...
      F = feval(InferenceDS.ffun, InferenceDS, S, cvecrep(V,InferenceDS.statedim+1), cvecrep(U1,InferenceDS.statedim+1));
      ...
      F = feval(InferenceDS.ffun, InferenceDS, cvecrep(state,InferenceDS.U1dim+1, cvecrep(V,InferenceDS.U1dim+1), U));
      ...
      F = feval(InferenceDS.ffun, InferenceDS, cvecrep(state,InferenceDS.Vdim+1), VV, cvecrep(U1,InferenceDS.Vdim+1));
      ...
      S = cvecrep(state, InferenceDS.statedim+1);
      S(:,2:end) = S(:,2:end)+diag(cvecrep(epsilon,InferenceDS.statedim));
      F = feval(InferenceDS.hfun, InferenceDS, S, cvecrep(N,InferenceDS.statedim+1), cvecrep(U2,InferenceDS.statedim+1));
      ...
      F = feval(InferenceDS.hfun, InferenceDS, cvecrep(state,InferenceDS.U2dim+1, cvecrep(N,InferenceDS.U2dim+1), U));
      ...
      F = feval(InferenceDS.hfun, InferenceDS, cvecrep(state,InferenceDS.Ndim+1), NN, cvecrep(U2,InferenceDS.Ndim+1));
