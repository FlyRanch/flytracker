...
%                and algorithm type. All ReBEL estimation algorithms take a inference data structure (InferenceDS),
...
%          InferenceDS         (InferenceD) updated inference data structure
%
%     See also
%     GENINFDS, GENNOISEDS
%
...
error(consistent(InferenceDS,'InferenceDS'));         %-- check for consistentency of InferenceDS data structure
...
case 'state'
    ...
    if stringmatch(estimatorType, 'gspf')   % 'Gaussian Sum Particle Filter'
        ...
        if ~stringmatch(pNoise.ns_type, 'gmm')
            ...
            if isfield(pNoise,'cov')
              ...
              otherwise
                  error('[ gensysnoiseds::gspf ] Unknown proces noise covariance type.');
              end
            else
    ...
    %----------------------------------------------------------------------------------
    if stringmatch(estimatorType, 'gmsppf')   % 'Sigma-Point Bayes Filter'
            ...
            if isfield(pNoise,'cov')
              ...
              switch cov_type
                ...
                case {'full','diag'}
                    Arg.cov(:,:,1) = chol(pNoise.cov)';
                otherwise
                    error(' [ gensysnoiseds::gmsppf ] Unknown proces noise covariance type.');
                end
...
case 'parameter'
    ...
    if stringmatch(estimatorType, {'kf','ekf','ukf','cdkf','srukf','srcdkf'})
        ...
        if stringmatch(InferenceDS.paramFunSelect, {'both-p','ffun'})
        ...
        if stringmatch(InferenceDS.paramFunSelect, {'both','both-p','hfun'})
            ...
            if ~stringmatch(InferenceDS.model.oNoise.ns_type, {'gaussian','combo-gaussian'})
                ...
                obs_oNoise = InferenceDS.model.oNoise; % or copy the original pnoise if its already gaussian
            end
        end
