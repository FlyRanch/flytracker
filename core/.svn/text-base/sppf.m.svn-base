...
for j=1:NOV,
    ...
    prior = feval(InferenceDS.prior, InferenceDS, xPred, x, UU1, pNoise) + 1e-99;

    % calculate observation likelihood for each particle (in log domain)
    likelihood = feval(InferenceDS.likelihood, InferenceDS, OBS, xPred, UU2, oNoise) + 1e-99;
    ...
    if pNoise.adaptMethod switch InferenceDS.inftype
            ...
            case 'parameter'  %--- parameter estimation
                switch pNoise.adaptMethod
                    ...
                    otherwise
                        error(' [ sppf ] Unkown process noise adaptation method!');
                end
