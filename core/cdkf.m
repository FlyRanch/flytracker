function [xh, Px, pNoise, oNoise, InternalVariablesDS] = cdkf(state, Pstate, pNoise, oNoise, Features, U1, U2, InferenceDS)
...
NOV = length(Features) - 1;%we have solution to first frame  % number of input vectors
...
for i=1:NOV,
  ...
  if U2dim, UU2 = cvecrep(U2(:,i),nsp2); end
    ...
    if isempty(sA_IdxVec)
        X_ = feval(InferenceDS.ffun, InferenceDS, Z(1:Xdim,:), Z(Xdim+1:Xdim+Vdim,:), UU1);  % propagate sigma-points through process model
    ...
    if isempty(oA_IdxVec)
        Y_ = feval(InferenceDS.hfun, InferenceDS, Z(1:Xdim,:), Z(Xdim+1:Xdim+Ndim,:), UU2);
    ...
    %-----------------------------------------------------
    % --- DATA ASSOCIATION
    % --- Take the current state and set 'obs' to the closest
    % points given the observation function
    
    %Look for closest head/tail points
    % This is the observation mean evaluated at the current state
    yyh_ = reshape(yh_(:,1),2,[])'; %[N 2] matrix to use in kdtree
    
    obs1 = kdtree(Features(i+1).Candpts , yyh_(1:2,:));
    obs2 = kdtree(Features(i+1).All_Bndy_Points , yyh_(3:end,:));
    
    %Reshape observation into one long vector, [x1;y1;x2;y2;...;xN;yN]
    
    obs(:,i) = reshape([obs1 ; obs2]',[],1);
    ...
    case 'parameter'  %--- parameter estimation
        ...
        otherwise
            error(' [ cdkf ] Unkown process noise adaptation method!');
        end
