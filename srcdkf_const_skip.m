function [xh, Sx, pNoise, oNoise, InternalVariablesDS,Features] = srcdkf_const_skip(state, Sstate, pNoise, oNoise, InferenceDS,frames,useIC)
%
% This function skip over the update step for a specified number of frames
% to test the quality of the prediction.
%
...
for i=frames(1):PAR.framesample:frames(2)
    ...
    %------------------------------------------------------
    [Features,IMBWCurr] = feat_detect(i,[],PAR);
    %%%%%%%%%%%%%%%%
    ...
    if i > PAR.ICframe+NumPrevSol-1
        InferenceDS.model.Fmode = 'predict';
        %load the previous solutions
        recalcIC_Joint([],i-NumPrevSol:i-2,PAR,'load');
        %recalcIC_MotionQNoise([],[],i-NumPrevSol:i-2,PAR,'load');
    else
...
%     [X_,InferenceDS] = feval(InferenceDS.ffun,InferenceDS,Z(1:Xdim,:),Z(Xdim+1:Xdim+Vdim,:), UU1);
...
%     Sx_= Sx_';
    ...
    if any(i == PAR.frames2skip)
        ...
        for n = 1:ITERMAX
            ...
            spidx = 1:15;
            resnrm(n) = (obs-yh)'*Sy*Sy'*(obs-yh) + (Xh_(spidx) - newstate(spidx))'*Sx_(spidx,spidx)*Sx_(spidx,spidx)'*(Xh_(spidx) - newstate(spidx));
    ...
    save([PAR.solutionpath 'fly_' PAR.stub '_FD_BodyScale/fly' num2str(i) '.mat'],'xh','InternalVariablesDS');
    save([PAR.solutionpath 'Features_' PAR.stub '_FD_BodyScale/Features' num2str(i) '.mat'],'Features');
