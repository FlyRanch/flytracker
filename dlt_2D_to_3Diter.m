function [XYZ,fval] = dlt_2D_to_3Diter( DLT, u, v )
...
[topt,fval] = fsolve(@ptfun,0,options);
XYZ = [polyval(params_x,topt);polyval(params_y,topt);polyval(params_z,topt)];

    function F = ptfun(tt)
        linpts =[polyval(params_x,tt);polyval(params_y,tt);polyval(params_z,tt)];
        
        uv = dlt_3D_to_2D(DLT,linpts');
