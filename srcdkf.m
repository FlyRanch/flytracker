function [xh, Sx, pNoise, oNoise, InternalVariablesDS,Features] = srcdkf(state, Sstate, pNoise, oNoise, InferenceDS,frames,useIC)
...
if (exist([PAR.solutionpath 'fly_' PAR.stub '/fly' num2str(frames(1)-PAR.framesample) ...
    ...
    %memory, unless I specify to use the IC that is input (useIC = true).
    ...
    save([PAR.solutionpath 'fly_' PAR.stub '/fly' num2str(0) '.mat'],'xh','InternalVariablesDS')
end
...
for i=frames(1):PAR.framesample:frames(2)
    ...
    if i > PAR.ICframe+NumPrevSol-1
        ...
        %recalcIC_JointBody([],[],i-NumPrevSol:i-2,PAR,'load');
    else
...
%             Syx1 = Syx1_prev{n-framebk};
    ...
    %Don't save the variables to memory if function is called with output
    %arguments
    if nargout == 0
        %--- Store Variables
