		 ...
		 int idx)
{
  ...
  if (idx <= npts)
    { /*the partial is with respect to a x control point*/
      d = 2*a;
      f = D1[idx-1];
    }
  else
  ...
  mxFree(N);
  mxFree(D1);
  V = (1 / (2*sqrt( pow(a,2) + pow(b,2))) ) *d*f ;
  return V;
}
  ...
  
{
  ...
  X[0] = qsimp(tangfunc1,A,B,ss,p,c,npts,x,idx);
...
{ 
  ...
  DLengthInt(X,p,c,T,npts,x,idx,ss); 
  mexAtExit(cleanup);
  return;
