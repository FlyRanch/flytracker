		   ...
		   
{
  ...
  double qromb(double (*func)(double, double *, double *, int, double *,int idx), 
	       double a,
	       double b,
	       double *ss,
	       double p[],
  ...
  for (i = 0; i <= sizeofT - 1; i++){
    ...
    X[i] = qromb(tangfunc1,0,t,ss,p,c,npts,x,idx);
    X[(i + sizeofT)] = qromb(tangfunc2,0,t,ss,p,c,npts,x,idx);
  }
  return;  
}
...
{ 
  ...
  DmodelRomIntP(X,p,c,T,npts,x,idx,sizeofT,ss); 
  mexAtExit(cleanup);
  return;
