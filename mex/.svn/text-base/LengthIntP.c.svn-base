...
double tangfunc1(double T,
		 ...
		 double x[])
{
  void dbasisP(double N[],double D1[], double *c, double T, int npts, double x[]);
  ...
  for (n=0; n <= npts-1; n++){
   ...
   /* Perform inner product between y-spline control points and basis function*/
  for (n=0; n <= npts-1; n++){
    b += D1[n] * p[n + npts];
  }
  mxFree(N);
